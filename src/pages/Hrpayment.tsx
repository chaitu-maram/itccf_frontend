...
export default function HRPayment() {
    ...
    return (
      ...
      >
        ...
        <div className="text-center space-y-5 w-full max-w-sm px-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">Payment Confirmed!</h1>
            <p className="text-sm font-semibold text-slate-400 mt-1">Your reference ID has been recorded.</p>
          </div>
          <div className="flex items-center gap-2 justify-center text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-full px-4 py-2 w-fit mx-auto">
            <Loader2 className="w-3.5 h-3.5 animate-spin" /> Redirecting to login…
  ...
  return (
    ...
    >
      ...
      {/* ── Navbar ── */}
      {/* RESPONSIVE: reduced height and padding on mobile */}
      <nav className="sticky top-0 z-30 h-14 sm:h-16 bg-white/95 backdrop-blur-md border-b-2 border-blue-100 shadow-sm shadow-blue-50">
        <div className="max-w-lg mx-auto px-3 sm:px-6 h-full flex items-center justify-between">
          <Link
            to="/verify"
            className="flex items-center gap-1.5 sm:gap-2 text-sm font-bold text-blue-400 hover:text-blue-600 transition-colors"
          >
            <ArrowLeft className="w-4 h-4" /> Back
          </Link>
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            <span className="text-sm sm:text-base font-black tracking-tight text-blue-900">Payment</span>
          </div>
      ...
      {/* RESPONSIVE: reduced padding on mobile, fluid vertical spacing */}
      <main className="max-w-lg mx-auto px-3 sm:px-4 py-6 sm:py-12 pb-16 sm:pb-24">
        ...
        <div className="text-center mb-6 sm:mb-10">
          ...
          <h1 className="text-xl sm:text-2xl font-black text-blue-900 tracking-tight">Complete Payment</h1>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-blue-100 shadow-lg shadow-blue-50 overflow-hidden">
          ...
          <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b-2 border-blue-50 bg-gradient-to-r from-blue-50 to-white">
            ...
            <span className="text-xs sm:text-sm font-black tracking-widest uppercase text-blue-700">Secure UPI Payment</span>
          </div>
          ...
          <div className="p-4 sm:p-8 space-y-5 sm:space-y-7">
            ...
            {/* ── Amount badge ── */}
            <div className="flex items-center justify-center">
              <div className="bg-gradient-to-r from-blue-600 to-sky-500 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-2xl shadow-lg shadow-blue-200">
                <p className="text-[10px] font-black tracking-[0.2em] uppercase opacity-80 mb-0.5">Amount Due</p>
                {/* RESPONSIVE: smaller font on mobile */}
                <p className="text-2xl sm:text-3xl font-black tracking-tight">{PAYMENT_AMOUNT}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-3 sm:gap-4">
              {/* RESPONSIVE: QR box scales with viewport */}
              <div className="relative p-2.5 sm:p-3 bg-white rounded-2xl border-2 border-blue-100 shadow-md shadow-blue-50">
                {/* Corner accents */}
                <div className="absolute top-2 left-2 w-5 h-5 border-t-2 border-l-2 border-blue-500 rounded-tl-lg pointer-events-none" />
                <div className="absolute top-2 right-2 w-5 h-5 border-t-2 border-r-2 border-blue-500 rounded-tr-lg pointer-events-none" />
                <div className="absolute bottom-2 left-2 w-5 h-5 border-b-2 border-l-2 border-blue-500 rounded-bl-lg pointer-events-none" />
                ...
                {/* RESPONSIVE: QR image size fluid — full on 360px, capped at 220px on larger */}
                <img
                  src={paymentQR}
                  alt="Payment QR Code"
                  className="rounded-xl block w-full max-w-[180px] sm:max-w-[220px] h-auto"
                  style={{ minWidth: 140 }}
                  // onError={e => {
                  //   (e.target as HTMLImageElement).src =
                  //     "https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=PAYMENT";
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-black tracking-widest uppercase text-blue-500 block">UPI ID</label>
              <div className="flex items-center gap-2">
                {/* RESPONSIVE: UPI text truncates gracefully on narrow screens */}
                <div className="flex-1 h-11 flex items-center gap-2.5 rounded-xl border-2 border-blue-100 bg-blue-50 px-3 sm:px-4 min-w-0">
                  <span className="text-xs sm:text-sm font-black text-blue-700 truncate">{upiId}</span>
                </div>
                <button
                  onClick={handleCopy}
                  className={`h-11 px-3 sm:px-4 rounded-xl border-2 text-xs font-black transition-all shrink-0 flex items-center gap-1 sm:gap-1.5 ${
                    copied
                      ? "border-emerald-300 bg-emerald-50 text-emerald-600"
                      : "border-blue-200 bg-blue-50 text-blue-500 hover:bg-blue-100 hover:border-blue-400"
                  }`}
            ...
            {/* ── Instruction note ── */}
            <div className="flex gap-2 sm:gap-3 bg-blue-50 border border-blue-200 rounded-xl px-3 sm:px-4 py-3">
              <AlertCircle className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
              <p className="text-xs font-semibold text-blue-500 leading-relaxed">
                After completing the payment, click <strong className="text-blue-700">Acknowledge Payment</strong> below and enter your UPI transaction reference number.
              </p>
            ...
            {phase === "qr" && (
              <button
                onClick={() => setPhase("ack")}
                className="w-full flex items-center justify-center gap-2 sm:gap-2.5 py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-blue-700 via-blue-600 to-sky-500 text-white text-sm font-black shadow-lg shadow-blue-200 hover:shadow-xl hover:shadow-blue-300 hover:-translate-y-0.5 active:translate-y-0 transition-all"
              >
                <ClipboardCheck className="w-4 h-4" />
                Acknowledge Payment
                <ChevronRight className="w-4 h-4" />
            ...
            {/* ── Reference ID form (shown in ack / submitting phase) ── */}
            {(phase === "ack" || phase === "submitting") && (
              <div className="space-y-4 border-t-2 border-blue-50 pt-5 sm:pt-6">

                <div className="flex items-center gap-2 mb-1">
                  <div className="w-6 h-6 rounded-lg bg-emerald-500 flex items-center justify-center shrink-0">
                    <CheckCircle className="w-3.5 h-3.5 text-white" />
                  </div>
                  <p className="text-xs sm:text-sm font-black text-slate-700">Payment Done? Enter Reference ID</p>
                </div>
                ...
                {/* RESPONSIVE: Back button shrinks, Confirm button fills remaining space */}
                <div className="flex gap-2">
                  <button
                    onClick={() => { setPhase("qr"); setError(""); }}
                    disabled={phase === "submitting"}
                    className="h-12 px-3 sm:px-4 rounded-xl border-2 border-blue-200 bg-blue-50 text-xs font-black text-blue-500 hover:bg-blue-100 hover:border-blue-400 transition-all disabled:opacity-40 flex items-center gap-1 sm:gap-1.5 shrink-0"
                  >
                    <RefreshCw className="w-3.5 h-3.5" /> Back
                  </button>
                  <button
                    onClick={handleSubmit}
                    disabled={phase === "submitting" || !refId.trim()}
                    className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2.5 py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 text-white text-xs sm:text-sm font-black shadow-lg shadow-emerald-200 hover:shadow-xl hover:shadow-emerald-300 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:translate-y-0"
                  >
                    {phase === "submitting"
                      ? <><Loader2 className="w-4 h-4 animate-spin" /> Confirming…</>
                      : <><CheckCircle className="w-4 h-4" /> <span className="hidden xs:inline">Confirm &amp; Continue</span><span className="xs:hidden">Confirm</span> <ChevronRight className="w-4 h-4" /></>
                    }
                  </button>
                </div>
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6 mt-6 sm:mt-8">
          <Link to="/verify" className="text-xs sm:text-sm font-black text-blue-500 hover:text-blue-700 hover:underline transition-colors">
            ← Back to Verify
          </Link>
          <div className="w-1 h-1 rounded-full bg-blue-200" />
          <Link to="/" className="text-xs sm:text-sm font-black text-blue-500 hover:text-blue-700 hover:underline transition-colors">
            Sign In →
          </Link>
        </div>
