...
//               <div className="flex justify-center gap-1.5 pt-1">
//                 {Array.from({ length: 6 }).map((_, i) => {
//                   const isFilled = otp[i] && otp[i] !== " ";
//                   return (
//                     <div key={i} className={`h-1 rounded-full transition-all duration-200 ${isFilled ? "w-5 bg-blue-500" : "w-3 bg-blue-100"}`} />
//                   );
//                 })}
//               </div>
//             </div>
    ...
    <div className="flex gap-1.5 xs:gap-2 justify-center">
      {digits.map((d, i) => (
        <input
          ...
          onKeyDown={e => handleKey(i, e)}
          onFocus={e => e.target.select()}
          className={`w-10 h-12 xs:w-11 xs:h-13 sm:w-12 sm:h-14 rounded-xl border-2 text-center text-xl xs:text-2xl font-black text-slate-800 transition-all duration-150 focus:outline-none focus:ring-4 focus:ring-blue-100 disabled:opacity-50 disabled:cursor-not-allowed ${
            d && d !== " "
              ? "border-blue-500 bg-blue-50 shadow-sm shadow-blue-100"
              : "border-blue-100 bg-blue-50 focus:border-blue-500 focus:bg-white"
          }`}
...
export default function HRVerify() {
  ...
  if (phase === "success") {
    return (
      ...
      >
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&display=swap');`}</style>
        <div className="text-center space-y-5 w-full max-w-xs sm:max-w-sm">
          <div className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-gradient-to-br from-emerald-500 to-teal-400 shadow-2xl shadow-emerald-200 mx-auto">
            <CheckCircle className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">Email Verified!</h1>
            <p className="text-xs sm:text-sm font-semibold text-slate-400 mt-1">Your HR account is now active.</p>
          </div>
          <div className="flex items-center gap-2 justify-center text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-full px-4 py-2 w-fit mx-auto">
            <Loader2 className="w-3.5 h-3.5 animate-spin" /> Redirecting to payment…
          </div>
  ...
  return (
    ...
    >
      ...
      <nav className="sticky top-0 z-30 h-14 sm:h-16 bg-white/95 backdrop-blur-md border-b-2 border-blue-100 shadow-sm shadow-blue-50">
        <div className="max-w-lg mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
          <Link
            to="/"
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center shadow-lg shadow-blue-200">
              <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
            </div>
            <span className="text-sm sm:text-base font-black tracking-tight text-blue-900">Verify Account</span>
          </div>
      ...
      <main className="max-w-lg mx-auto px-4 sm:px-6 py-8 sm:py-12 pb-16 sm:pb-20">
        ...
        <div className="text-center mb-7 sm:mb-10">
          ...
          <h1 className="text-xl sm:text-2xl font-black text-blue-900 tracking-tight">Verify Your Email</h1>
          <p className="text-xs sm:text-sm font-semibold text-blue-400 mt-1">Enter the 6-digit OTP sent to your inbox</p>
        </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-blue-100 shadow-lg shadow-blue-50 overflow-hidden">
          ...
          <div className="p-5 sm:p-8 space-y-5 sm:space-y-7">
            ...
            {/* ── Email row ── */}
            <div className="space-y-1.5">
              <label className="text-[10px] sm:text-xs font-black tracking-widest uppercase text-blue-500 block">
                Registered Email
              </label>
              {emailLocked ? (
                <div className="flex items-center gap-2">
                  <div className="flex-1 min-w-0 h-10 sm:h-11 flex items-center gap-2 sm:gap-2.5 rounded-xl border-2 border-blue-100 bg-blue-50 px-3 sm:px-4">
                    <Mail className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-300 shrink-0" />
                    <span className="text-xs sm:text-sm font-black text-blue-700 truncate">{email}</span>
                  </div>
                  <button
                    onClick={handleChangeEmail}
                    className="h-10 sm:h-11 px-3 sm:px-4 rounded-xl border-2 border-blue-200 bg-blue-50 text-[10px] sm:text-xs font-black text-blue-500 hover:bg-blue-100 hover:border-blue-400 transition-all shrink-0 whitespace-nowrap"
                  >
                    Change
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="your@email.com"
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)}
                    className="flex-1 min-w-0 h-10 sm:h-11 rounded-xl border-2 border-blue-100 bg-blue-50 px-3 sm:px-4 text-sm sm:text-base font-semibold text-slate-800 placeholder:text-blue-200 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100 transition-all"
                  />
                  {email && (
                    <button
                      onClick={() => { setEmailLocked(true); restartCooldown(60); }}
                      className="h-10 sm:h-11 px-3 sm:px-4 rounded-xl bg-blue-600 text-white text-[10px] sm:text-xs font-black hover:bg-blue-700 transition-colors shrink-0 whitespace-nowrap"
                    >
                      Confirm
                    </button>
                  )}
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="h-px flex-1 bg-blue-100" />
              <span className="text-[9px] sm:text-[10px] font-black tracking-[0.2em] sm:tracking-[0.25em] uppercase text-blue-400 px-2 bg-blue-50 border border-blue-100 rounded-full py-0.5 whitespace-nowrap">
                Enter OTP
              </span>
              <div className="h-px flex-1 bg-blue-100" />
            </div>
              ...
              <div className="flex justify-center gap-1.5 pt-1">
                {Array.from({ length: 6 }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`h-1 rounded-full transition-all duration-200 ${isFilled ? "w-5 bg-blue-500" : "w-3 bg-blue-100"}`}
                    />
                  );
                })}
              </div>
            ...
            {/* ── Error ── */}
            {error && (
              <div className="flex items-center gap-2 text-red-500 text-xs font-bold justify-center bg-red-50 border border-red-200 rounded-xl px-3 sm:px-4 py-2.5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-y-2 gap-x-3 pt-1">
              ...
              {cooldown > 0 ? (
                <span className="text-[11px] sm:text-xs font-black text-blue-400 bg-blue-50 border border-blue-100 px-2.5 sm:px-3 py-1 rounded-full">
                  Resend in {cooldown}s
                </span>
              ) : (
                <button
                  onClick={handleResend}
                  disabled={busy || !email || !emailLocked}
                  className="flex items-center gap-1.5 text-[11px] sm:text-xs font-black text-blue-600 hover:text-blue-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {phase === "resending" ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : (
        ...
        {/* ── Footer links ── */}
        <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 mt-6 sm:mt-8">
          <Link
            to="/signup"
            className="text-xs sm:text-sm font-black text-blue-500 hover:text-blue-700 hover:underline transition-colors"
          >
            ← Back to Sign Up
          </Link>
          <div className="w-1 h-1 rounded-full bg-blue-200 hidden sm:block" />
          <Link
            to="/"
            className="text-xs sm:text-sm font-black text-blue-500 hover:text-blue-700 hover:underline transition-colors"
          >
