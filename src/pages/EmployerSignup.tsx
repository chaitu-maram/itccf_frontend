  ...
  <div className="flex flex-col">
    ...
    <div className="relative">
      ...
      <textarea
        value={value} placeholder={placeholder ?? label} rows={3}
        onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onChange(field, e.target.value)}
        className={`w-full rounded-xl border-2 border-blue-100 bg-blue-50 px-4 py-3 text-sm sm:text-base font-semibold text-slate-800 placeholder:text-blue-200 transition-all duration-150 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100 resize-none ${icon ? "pl-10" : ""}`}
      />
    </div>
  </div>
);
...
const PhoneField = ({ value, onChange }: { value: string; onChange: (f: string, v: string) => void }) => {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between mb-1">
        <label className={labelCls + " mb-0"}>Contact Number</label>
        <span className={`text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-full border transition-all ${complete ? "text-emerald-700 bg-emerald-50 border-emerald-200" : "text-blue-300 bg-blue-50 border-blue-100"}`}>
          {digits.length}/10
        </span>
      </div>
      <div className="flex gap-2">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-blue-100 shadow-lg shadow-blue-50 overflow-hidden">
          ...
          <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
            ...
            {step === 1 && (
              <>
                ...
                {/* Company Name + Industry — stacked on mobile, side by side on sm+ */}
                ...
                {/* City + Pincode — stacked on mobile */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="City" field="companyCity" value={v("companyCity")} onChange={set}
                    icon={<MapPin className="w-4 h-4" />} placeholder="City" />
                  <Field label="Pincode" field="companyPincode" value={v("companyPincode")}
          ...
          {!isOtpStep && (
            <div className="px-4 sm:px-6 pb-5 sm:pb-6 flex items-center gap-2 sm:gap-3">
              ...
              {step < STEPS.length - 1 ? (
                <button
                  onClick={() => { if (stepValid()) setStep(s => s + 1); }}
                  disabled={!stepValid()}
