...
// Academics that have NO specialization at all
const NO_SPEC = new Set(["Below SSC", "SSC"]);

// Academics that also show a Sub Course field
...
/* ═══════════════════════════════════════════════════════════
   SpecField — specialization dropdown fetched from API
═══════════════════════════════════════════════════════════ */
interface SpecFieldProps { academic: string; value: string; onChange: (f: string, v: string) => void; }
const SpecField = ({ academic, value, onChange }: SpecFieldProps) => {
  const [options, setOptions] = useState<string[]>([]);
...
/* ═══════════════════════════════════════════════════════════
   SubCourseField — shown only for U.G (Degree) / P.G (Degree)
   Fetches sub_course values from the same degree API endpoint
═══════════════════════════════════════════════════════════ */
interface SubCourseFieldProps {
  academic: string;
  specialization: string;   // filter sub-courses by selected specialization
  value: string;
...
const SubCourseField = ({ academic, specialization, value, onChange }: SubCourseFieldProps) => {
  ...
  // Fetch all records once when academic changes
  ...
  // Re-filter options whenever specialization changes
  useEffect(() => {
    onChange("subCourse", "");
    if (!specialization) {
      // Show all sub-courses if no specialization picked yet
...
const buildFormData = (form: F, hrPhoto: File | null, studentPhoto: File | null, signature: File | null): FormData => {
  ...
  a("regn_no", form.regnNo);          a("date", form.date);
  a("academic", form.academic);       a("specialization", form.specialization);
  a("sub_course", form.subCourse);    // ← new field
  a("extra_curricular", form.extraCurricular);
  a("additional_qualification", form.additionalQualification);
  a("driving_licence", form.drivingLicence);
  a("aadhar_no", form.aadharNo);
...
const StudentProfile = () => {
  ...
  // Determine grid columns for qualification row dynamically
  const qualColCount = [true, showSpec, showSubCourse].filter(Boolean).length;
  const qualGridCls  = qualColCount === 3 ? "md:grid-cols-3" : qualColCount === 2 ? "md:grid-cols-2" : "md:grid-cols-1";
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-50"
      ...
      <nav className="sticky top-0 z-30 h-16 bg-white/95 backdrop-blur-md border-b-2 border-blue-100 shadow-sm shadow-blue-50">
        <div className="max-w-5xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <span className="text-[10px] font-black tracking-widest text-blue-600 uppercase bg-blue-50 border border-blue-200 px-3 py-1 rounded-full">Form</span>
        </div>
      </nav>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-5 pb-28">
        ...
        <Card>
          ...
          <div className="p-6">
            ...
            {/* Row 1: Academic + Specialization (if applicable) + Sub Course (if applicable) */}
