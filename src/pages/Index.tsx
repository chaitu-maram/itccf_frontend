...
//   .main-grid{display:grid;grid-template-columns:300px minmax(0,1fr) 260px;gap:16px;align-items:start;}
//   @media(max-width:1200px){.main-grid{grid-template-columns:280px 1fr !important;}.yt-col{display:none !important;}}
//   @media(max-width:820px){.main-grid{grid-template-columns:1fr !important;}.page-pad{padding:16px 14px !important;}}
...
/* ══════ LOGIN CARD ════════════════════════════════════════════ */
// "mode" drives the card: "login" | "fp_id" | "fp_otp" | "fp_pw"
...
//   // ── Forgot-password step callbacks ──
...
//   // ── Gradient header config per mode ──
...
//   // ── Step indicator (shown only in forgot-pw modes) ──
...
//           style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", padding:0, color:"#7C3AED", display:"flex", alignItems:"center" }}
//         >
//           {showPw ? (
//             <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
//               <path d="M17.94 17.94A10.94 10.94 0 0 1 12 20C7 20 2.73 16.11 1 12a10.94 10.94 0 0 1 3.06-4.94"/>
//               <path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c5 0 9.27 3.89 11 8a10.9 10.9 0 0 1-1.25 2.24"/>
...
//   const [id,      setId]      = useState("");
//   const [pw,      setPw]      = useState("");
//   const [showPw,  setShowPw]  = useState(false);   // ✅ moved here
//   const [loading, setLoading] = useState(false);
//   const [error,   setError]   = useState("");
...
//   const headerTitle = mode === "login" ? "Login for HR & Employer"
//     : mode === "fp_id"  ? "Account Recovery"
//     : mode === "fp_otp" ? "OTP Verification"
//     : "Reset Password";

//   const headerSub = mode === "login" ? "Sign in to access your dashboard"
//     : mode === "fp_id"  ? "We'll send a one-time code to your email"
//     : mode === "fp_otp" ? "Enter the code we just sent"
//     : "Create a new secure password";
...
//   // ── NO useState calls below this line ──
//   return (
//     <div style={{ borderRadius:20, overflow:"hidden", background:"#FFFFFF", border:"1px solid #DDD6FE", boxShadow:cardShadow }}>
...
//         {/* ════ FORGOT PW STEPS ════ */}
//         {mode === "fp_id" && (
//           <FPStepIdentifier onSent={handleFPSent} onBack={() => setMode("login")}/>
//         )}
//         {mode === "fp_otp" && (
//           <FPStepOTP identifier={fpIdentifier} maskedEmail={fpMasked} onVerified={handleFPVerified} onBack={() => setMode("fp_id")}/>
//         )}
//         {mode === "fp_pw" && (
//           <FPStepNewPassword identifier={fpIdentifier} resetToken={fpToken} onDone={handleFPDone}/>
//         )}
//       </div>
//     </div>
//   );
...
//   return (
//     <div style={{ borderRadius:20, overflow:"hidden", minWidth:0, background:"#FFFFFF", border:"1px solid #DDD6FE", boxShadow:cardShadow, display:"flex", flexDirection:"column" }}>
//       {/* Header */}
//       <div style={{ background:"linear-gradient(135deg,#4C1D95 0%,#7C3AED 50%,#A855F7 100%)", padding:"18px 20px", position:"relative", overflow:"hidden", flexShrink:0 }}>
//         <div style={{ position:"absolute", inset:0, background:"linear-gradient(135deg,rgba(255,255,255,0.18) 0%,rgba(255,255,255,0.04) 50%,transparent 100%)", pointerEvents:"none" }}/>
//         <div style={{ position:"absolute", right:-30, top:-30, width:120, height:120, borderRadius:"50%", background:"rgba(255,255,255,0.07)", pointerEvents:"none" }}/>
...
//           <LoginCard/>
//           <JobGrid/>
//           <div className="yt-col" style={{ borderRadius:20, minWidth:0, overflow:"hidden", background:"rgba(255,255,255,0.85)", border:"1px solid #DDD6FE", boxShadow:cardShadow }}>
//             <div style={{ background:"linear-gradient(135deg,#4C1D95 0%,#7C3AED 55%,#A855F7 100%)", padding:"18px 20px", position:"relative", overflow:"hidden" }}>
//               <div style={{ position:"absolute", inset:0, background:"linear-gradient(135deg,rgba(255,255,255,0.18) 0%,rgba(255,255,255,0.04) 60%,transparent 100%)", pointerEvents:"none" }}/>
//               <p style={{ margin:0, fontSize:15, fontWeight:700, color:"#fff", letterSpacing:"-0.3px", position:"relative" }}>Career Videos</p>
//               <p style={{ margin:"3px 0 0", fontSize:11, color:"rgba(255,255,255,0.70)", position:"relative" }}>Watch &amp; get inspired</p>
...
// const BASE = "http://10.82.220.163:8000/api";
...
//       {/* ── Header ── */}
//       <div style={{ background:"linear-gradient(135deg,#4C1D95 0%,#7C3AED 55%,#A855F7 100%)", padding:"22px 20px 18px", overflow:"hidden" }}>
//         <div style={{ position:"absolute", inset:0, background:"linear-gradient(135deg,rgba(255,255,255,0.18) 0%,rgba(255,255,255,0.04) 60%,transparent 100%)", pointerEvents:"none" }}/>
//         <div style={{ position:"absolute", right:-22, top:-22, width:90, height:90, borderRadius:"50%", background:"rgba(255,255,255,0.08)", pointerEvents:"none" }}/>
//         <div style={{ position:"absolute", left:-10, bottom:-20, width:60, height:60, borderRadius:"50%", background:"rgba(196,181,253,0.15)", pointerEvents:"none" }}/>
//         <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative" }}>
...
const G = `
  ...
  * { font-family: 'Outfit', system-ui, sans-serif !important; }
  ...
  @media(max-width:1280px){
    .main-grid{grid-template-columns:300px 1fr 280px !important;}
  }
  @media(max-width:1100px){
    .main-grid{grid-template-columns:290px 1fr !important;}
    .yt-col{display:none !important;}
  }
  @media(max-width:820px){
    .main-grid{grid-template-columns:1fr !important;}
  ...
  /* pw-field: password input wrapper */
...
function LoginCard() {
  ...
  const [id,      setId]      = useState("");
  const [pw,      setPw]      = useState("");
  const [showPw,  setShowPw]  = useState(false);   // ← NEW: eye toggle state
  const [loading, setLoading] = useState(false);
  const [error,   setError]   = useState("");
